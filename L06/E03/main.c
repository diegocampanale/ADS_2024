...
int eliminaPG(tabPg_t *tabPg){
    ...
    printf("Codice personaggio da cancellare: ");
    scanf(" %s",codice);
    if(codice[0]!='P'&&codice[1]!='G'&& isdigit(codice[2])&& isdigit(codice[3])&& isdigit(codice[4])&& isdigit(codice[5]))
        rimuoviPG(tabPg,codice);
    else
        printf("Formato codice non valido. Nessun personaggio eliminato.\n");
    return 0;
}